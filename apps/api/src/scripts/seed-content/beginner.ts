    ...
    questions: [
      {
        ...
        options: [
          ...
          { id: "user-opened-it", label: "The user opened the file after download. The download row's `state=completed` field on this browser version captures both the transfer and the post-download open event, so a `completed` state on a PDF row means the user actually viewed it." },
          { id: "user-shared-it", label: "The user shared the file outside the organization. Browser download history correlates with the upload-share telemetry the browser collects in parallel, and a downloaded-then-shared sequence appears as a single contiguous chain in the history view." },
      ...
      },
      {
        ...
        options: [
          ...
          { id: "browser-history-again", label: "More entries from the browser download history — additional rows referring to the same file capture the open events the browser logs as separate transitions through its post-download lifecycle." },
          { id: "smtp-logs", label: "Outbound SMTP logs from the mail server — once the user opens a downloaded document, most clients fire a delivery-receipt event back through the mail relay, so the SMTP server's outbound log will show the open as a per-document event." },
        ],
        allowMultiple: true,
        expected: {
          type: "multi_choice",
